  ...
  let build_function_body fdecl =
    ...
    let rec stmt m dl builder = function
      ...
      | SIf (predicate, then_stmt, else_stmt) ->
        ...
        let (builder, _, _) = stmt m dl (L.builder_at_end context then_bb) then_stmt in
        add_terminal builder build_br_merge;

        let else_bb = L.append_block context "else" the_function in
        let (builder, _, _) = stmt m dl (L.builder_at_end context else_bb) else_stmt in
        add_terminal builder build_br_merge;
    ...
    | SDefer(_, SCall(f, args)) -> 
      ...
      let result = (match fdecl.stypes with 
                          ...
                          | _ :: [] -> f ^ "_result"
                          | _ -> raise(Failure("Multiple return types not implemented yet"))) in
      (builder, m, (fdef, llargs, result)::dl)
    in

    (* Build the code for each statement in the function *)
    
    let (builder, _, _) = stmt [local_vars ; global_vars] defered_el builder (SBlock fdecl.sbody) in
